...
__package__ = "build_CryptoSocket"
...
def read_version (CMakeLists):
  ...
  with open(CMakeLists, 'r') as fp:
    ...
    for row in fp:
      ...
      if 'CRYPTO_MAJOR' in row:
        version.append(row.split('CRYPTO_MAJOR')[-1])

      elif 'CRYPTO_MINOR' in row:
        version.append(row.split('CRYPTO_MINOR')[-1])

      elif 'CRYPTO_REVISION' in row:
        version.append(row.split('CRYPTO_REVISION')[-1])
...
class cryptosocket_build_ext (build_ext):
  '''
  Custom build type
  '''
